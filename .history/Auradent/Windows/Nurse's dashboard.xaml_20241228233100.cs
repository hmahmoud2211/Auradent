    ...
    public partial class Nurse_s_dashboard : Window
    {
        ...
        private void UpdateCurrentMonthYear()
        {
            if (mainCalendar.DisplayDate != null)
            {
                ...
                var yearPanel = currentYearButton.Parent as StackPanel;
                if (yearPanel != null)
                {
                    foreach (var child in yearPanel.Children)
                    {
                        if (child is Button btn && btn.Tag != null && int.TryParse(btn.Tag.ToString(), out int btnYear))
                        {
                ...
                var monthPanel = LogicalTreeHelper.GetParent(currentYearButton) as WrapPanel;
                if (monthPanel != null)
                {
