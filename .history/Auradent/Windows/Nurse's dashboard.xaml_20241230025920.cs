...
namespace Auradent.Windows
{
    ...
    public partial class Nurse_s_dashboard : Window
    {
        ...
        private void LoadSelectedDate(DateTime date)
        {
            ...
            var appointmentItems = appointments.Select(a =>
            {
                ...
                return new AppointmentViewModel
                {
                    ...
                    StatusIcon = a.AppointmentStatus == "Completed" ? "CheckCircle" : "Circle",
        ...
        private void Appointment_CheckBox_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is AppointmentViewModel appointment)
            {
                try
                {
                    var appointmentToUpdate = _appointmentHelper.GetAllData()
                        .FirstOrDefault(a => a.AppointmentID == appointment.AppointmentId);

                    if (appointmentToUpdate != null)
                    {
                        appointmentToUpdate.AppointmentStatus = appointmentToUpdate.AppointmentStatus == "Completed" ? "Pending" : "Completed";
                        _appointmentHelper.Update(appointmentToUpdate);
                        LoadAppointments(); // Refresh the list
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Error updating appointment status: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
    }
