    ...
    public partial class Nurse_s_dashboard : Window
    {
        ...
        private void LoadSelectedDate(DateTime date)
        {
            ...
            var appointmentItems = appointments.Select(a =>
            {
                ...
                var itemViewModel = new AppointmentItemViewModel
                {
                ...
                var buttonViewModel = new AppointmentViewModel
                {
                ...
                return new { ItemViewModel = itemViewModel, ButtonViewModel = buttonViewModel };
            }).ToList();

            // Update appointments list
            appointmentsList.ItemsSource = appointmentItems.Select(x => x.ItemViewModel);

            // Update button tags
            var appointmentButtons = appointmentsList.ItemsSource
                .Cast<AppointmentItemViewModel>()
                .Select((item, index) => new { Item = item, ButtonViewModel = appointmentItems[index].ButtonViewModel })
                .ToList();

            foreach (var button in appointmentButtons)
            {
                var container = appointmentsList.ItemContainerGenerator.ContainerFromItem(button.Item) as ContentPresenter;
                if (container != null)
                {
                    var template = container.ContentTemplate;
                    var editButton = template.FindName("EditButton", container) as Button;
                    var deleteButton = template.FindName("DeleteButton", container) as Button;

                    if (editButton != null)
                        editButton.Tag = button.ButtonViewModel;
                    if (deleteButton != null)
                        deleteButton.Tag = button.ButtonViewModel;
                }
            }
        }
                ...
                if (result == MessageBoxResult.Yes)
                {
                    try
                    {
                        ...
                        if (appointmentToDelete != null)
                        {
                            ...
                            // Refresh the current date's appointments
                            if (mainCalendar.SelectedDate.HasValue)
                            {
                                LoadSelectedDate(mainCalendar.SelectedDate.Value);
                            }
