...
namespace Auradent.Windows.patient_finance
{
    ...
    public partial class Patient_Finance : Window
    {
        ...
        private void LoadTransactions()
        {
            ...
            // First get all appointments for this patient with their finance records
            var patientAppointments = _appointmentHelper.GetAllData()
                .Where(a => a.PatientID == _patient.PatientID)
                .Join(_financeHelper.GetAllData(),
                    apt => apt.Fainance_Fk,
                    fin => fin.FinanceId,
                    (apt, fin) => new { Appointment = apt, Finance = fin })
                .OrderByDescending(x => x.Appointment.AppointmentDate)
                .ToList();

            foreach (var record in patientAppointments)
            {
                _transactions.Add(new FinanceViewModel
                {
                    Month = record.Appointment.AppointmentDate.ToString("MMM dd, yyyy"),
                    Sales = record.Finance.TotalAmount.ToString("C"),
                    Status = record.Finance.TypeOfPayment,
                    RawAmount = record.Finance.TotalAmount,
                    PaymentMethod = record.Finance.PaymentMethod
                });
            }
